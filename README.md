...
-- **登录与基础信息**
  - 管理员登录：系统不再包含默认演示账号，请使用管理员创建的账户或根据部署要求手动插入管理员记录。
  - 获取当前用户信息、退出登录
...
- **后端**
  - Spring Boot **3.4.1**
  - Spring Web
  - Spring Security（简单放行规则，登录逻辑基于数据库与 JWT）
...
登录接口：`POST /swiftstock/auth/login`。后端基于数据库账号进行认证，成功后返回 JWT（见 `JwtTokenUtil`），前端将 token 存入 `localStorage` 并在路由守卫与 Axios 拦截器中使用。请在部署前为管理员创建账户或通过迁移脚本导入。
